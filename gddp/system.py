...
class System(object):
    ...
    def __init__(self, name=None, model_dict=None):
        ...
        """
        if name is None and model_dict is None:
            print "No system name or parameter dictionary specified!"
            raise SystemExit()
        ...
        if name == "1D":
            ...
            self.phi_i_x_const = (0., 0.)  # Const component of state cost
            self.phi_i_x_lin = (np.array([0.]), np.array([0.]))  # Linear component of state cost
            self.phi_i_x_quad = (np.array([[1.]]), np.array([[0.]]))  # 2 * Hessian of state cost
            self.Ri = (np.array([[4.]], dtype=float),
                       np.array([[0]], dtype=float),)  # 2 * input cost Hessian
            ...
            self.brute_force_solve = False  # Solve one-stage problems by brute force (no solver)
        elif name == "Double integrator":
            self.m = 1
            self.n = 2
            self.gamma = 0.95
            tau = 0.1
            ...
            self.pure_lqr = False
            self.brute_force_solve = True
        elif name == "8states3inputs":
            mat_saved = loadmat('Matlab/8smodel.mat')
            self.m = 3
            self.n = 8
            self.gamma = 0.95
            self.A_mat = mat_saved['A']
            self.B_mat = mat_saved['B']
            ...
            self.A_mat = None
            self.B_mat = None
            self.D = np.zeros((2, self.n), dtype=float)  # Dx + Eu <= h
            self.E = np.array([[-1.], [1.]], dtype=float)
            min_u, max_u = -9, 9
            self.h = np.array([-min_u, max_u], dtype=float)
            self.n_beta = 1
            ...
            self.phi_i_x_const = (0.,)
            self.phi_i_x_lin = (np.zeros((self.n,), dtype=float),)
            self.phi_i_x_quad = (np.array([[1., 0.], [0., 0.]], dtype=float),)
            input_cost = 0.01
            self.Ri = (input_cost * np.eye(self.m, dtype=float),)  # 2 * input cost Hessian
            self.ri = (np.zeros((self.m,), dtype=float),)  # input cost linear component
            self.ti = (0.,)  # constant cost component
            self.state_mean = np.array([np.pi, 0], dtype=float)
            self.state_var = np.diag([0.5 * np.pi ** 2, (0.5 * np.pi) ** 2])
            self.physics = {'m': 1.0, 'g': 9.81, 'l': 1.0, 'ft': 2.0, 'dt': 0.1}
            self.pure_lqr = False
            self.brute_force_solve = True
        elif name == "Inverted pendulum 2":
            # https://www.control.isy.liu.se/student/tsrt19/ht2/file/invpendpmenglish.pdf
            self.m = 1
            self.n = 2
            self.gamma = 1.00
            ...
            self.state_mean = np.array([np.pi, 0], dtype=float)
            self.state_var = np.diag([0.5 * np.pi ** 2, (0.5 * np.pi) ** 2])
            self.physics = {'m': 1.0, 'g': 9.81, 'l': 1.0, 'ft': 2.0, 'dt': 0.2}
            self.pure_lqr = False
            self.brute_force_solve = True
        elif name == "Inverted pendulum on cart":
            # https://www.control.isy.liu.se/student/tsrt19/ht2/file/invpendpmenglish.pdf
            self.m = 1
            self.n = 4
            self.gamma = 0.95
            ...
            self.ti = (0.,)  # constant cost component
            self.state_mean = np.zeros((self.n,), dtype=float)
            self.state_var = np.diag([1., 1., 0.5 * np.pi, 0.5 * np.pi])
            self.physics = {'m': 1.0, 'M': 5000.0, 'g': 9.81, 'l': 1.0, 'ft': 1.0, 'dt': 0.05}
            self.pure_lqr = False
            self.brute_force_solve = True
        elif name == "Bead on beam":
            self.m = 1
            self.n = 4
            self.gamma = 0.95
            self.A_mat = None
            ...
            quadratic_state_cost = True
            if quadratic_state_cost:
                self.n_beta = 1
                self.li = (np.array([1.]),)  # selector row matrix for cost epigraph
                self.phi_i_x_const = (0.,)
                self.phi_i_x_lin = (np.array([0., 0., 0., 0.]),)
                self.phi_i_x_quad = (np.diag([10., 1., 1., 1.]),)
                input_cost = 0.01
                self.Ri = (np.array([[input_cost]]),)  # 2 * input cost Hessian
                self.ri = (np.array([0.]),)  # input cost linear component
                self.ti = (0.,)  # constant cost component
            else:
                self.n_beta = 3
                self.li = (np.array([1., 0., 0.]),
                           np.array([1., 0., 0.]),
                           np.array([0., 1., 0.]),
                           np.array([0., 1., 0.]),
                           np.array([0., 0., 1.]))  # selector row matrix for cost epigraph
                self.phi_i_x_const = (0., 0., 0., 0., 0.)
                self.phi_i_x_lin = (np.array([1., 0., 0., 0.]), np.array([-1., 0., 0., 0.]),
                                    np.array([0., 1., 0., 0.]), np.array([0., -1., 0., 0.]),
                                    np.array([0., 0., 0., 0.]))
                self.phi_i_x_quad = (np.diag([0., 0., 0., 0.]), np.diag([0., 0., 0., 0.]),
                                     np.diag([0., 0., 0., 0.]), np.diag([0., 0., 0., 0.]),
                                     np.diag([0., 0., 0., 0.]))
                input_cost = 0.01
                self.Ri = (np.array([[0.]]), np.array([[0.]]), np.array([[0.]]), np.array([[0.]]),
                           np.array([[input_cost]]))  # 2 * input cost Hessian
                self.ri = (np.array([0.]), np.array([0.]), np.array([0.]), np.array([0.]),
                           np.array([0.]))  # input cost linear component
                self.ti = (0., 0., 0., 0., 0.)  # constant cost component

            # State sampling parameters
            self.state_mean = np.array([0.0, 0.0, 0.0, 0.0])
            self.state_var = np.diag([a * a for a in [0.5, 0.5, 0.5, 0.5]])
            self.physics = {'M': 0.1, 'g': 9.81, 'J': 0.5, 'f': 0.0, 'dt': 0.1}
            self.pure_lqr = False
            self.brute_force_solve = True
    ...
    def set_discount(self, gamma_in):
        # Set discount factor to a new value
        self.gamma = gamma_in
    ...
    def f_x_x(self, x=None):
        ...
        if x is not None:
            if self.A_mat is not None:
            ...
            elif self.name == "Inverted pendulum" or self.name[:2] == "IP":
                # https://www.control.isy.liu.se/student/tsrt19/ht2/file/invpendpmenglish.pdf
                m = self.physics['m']
                g = self.physics['g']
                l = self.physics['l']
                ...
                dt = self.physics['dt']
                x1dot = x[1]
                x2dot = g * np.sin(x[0]) - ft/(m * l) * x[1]
                return x + np.array([x1dot * dt, x2dot * dt])
            elif self.name == "Inverted pendulum on cart":
                # https://www.control.isy.liu.se/student/tsrt19/ht2/file/invpendpmenglish.pdf
                M = self.physics['M']
                m = self.physics['m']
                g = self.physics['g']
                ...
                ft = self.physics['ft']
                dt = self.physics['dt']
                den = M + (1 - np.cos(x[2]) ** 2) * m
                x1dot = x[1]
                x2dot = (-m * g * np.sin(x[2]) * np.cos(x[2]) +
                         m * l * x[3] * x[3] * np.sin(x[2]) +
                         ft * m * x[3] * np.cos(x[2])) / den
                x3dot = x[3]
                x4dot = ((M + m) * (g * np.sin(x[2]) - ft * x[3]) -
                         l * m * x[3] * x[3] * np.sin(x[2]) * np.cos(x[2])) / (l * den)
                return x + np.array([x1dot * dt, x2dot * dt, x3dot * dt, x4dot * dt], dtype=float)
            elif self.name == "Bead on beam":
                M = self.physics['M']
                g = self.physics['g']
                J = self.physics['J']
                f = self.physics['f']
                dt = self.physics['dt']
                x1dot = x[1]
                x2dot = x[0] * x[3] * x[3] - g * np.sin(x[2]) - f * x[1]
                x3dot = x[3]
                den = M * x[0] * x[0] + J
                x4dot = (-2 * M * x[0] * x[1] * x[3] - M * g * x[0] * np.cos(x[2]))  / den
                return x + np.array([x1dot * dt, x2dot * dt, x3dot * dt, x4dot * dt])
            else:
    ...
    def f_u_x(self, x):
        ...
        if x is not None:
            if self.B_mat is not None:
                ...
                dt = self.physics['dt']
                x2dot_expr = 1 / (m * l)
                self.B_mat = np.array([[0.], [x2dot_expr * dt]], dtype=float)
                return self.B_mat
            elif self.name == "Inverted pendulum on cart":
                # https://www.control.isy.liu.se/student/tsrt19/ht2/file/invpendpmenglish.pdf
                M = self.physics['M']
                ...
                den = M + (1 - np.cos(x[2]) ** 2) * m
                x2dot_expr = 1./den
                x4dot = -np.cos(x[2]) / (l * den)
                return np.array([[0.], [x2dot_expr * dt], [0.], [x4dot * dt]], dtype=float)
            elif self.name == "Bead on beam":
                M = self.physics['M']
                J = self.physics['J']
                dt = self.physics['dt']
                den = M * x[0] * x[0] + J
                x4dot = 1.0 / den
                return np.array([[0.], [0.], [0.], [x4dot * dt]])
            else:
    ...
    def phi_i_x(self, i, x=None):
        ...
        if x is not None:
            if self.name[:2] == "IP":
                if self.bf_type == "gaussian":
                    # 1. Scale Gaussian to account for std dev
                    mean = self.phi_bf_locs[i]
                    std = self.phi_bf_stds[i]
                    return np.exp(-0.5 / (std ** 2) * np.dot(x - mean, (x - mean)))
                else:
                    print "Unrecognized basis function type: " + self.bf_type
                    raise SystemExit()
            elif self.name == "Inverted pendulum":
                return 1.0 - np.cos(divmod(x[0] - np.pi, 2 * np.pi)[1] + np.pi)
            elif self.name == "Inverted pendulum on cart":
                return 1.0 - np.cos(divmod(x[2] - np.pi, 2 * np.pi)[1] + np.pi)
            else:
    ...
    def reachable_regions(self, rd, obs_bounds=None):
        # Return dictionary of form {region_id: [reachable_region_id1, reachable_region_id2, ...]}
        # where the list entries are regions which can be reached in one step from some point within
        # region region_id.
        # At present this function doesn't take system dynamics into account and just assumes that
        # reachable regions are those that are adjacent. This is the case for single-integrator sys.
        assert isinstance(rd, dict)
        assert rd[0][0].shape[1] == self.n, "Region dimensions don't match state space"
        neighbours = {}
        if self.n == 2:
            for r, rv in rd.iteritems():
                neighbours[r] = []  # Empty list of neighbouring regions of region r
                lb1, ub1, lb2, ub2 = -rv[1][1], rv[1][0], -rv[1][3], rv[1][2]
                for k, v in rd.iteritems():
                    lb1n, ub1n, lb2n, ub2n = -v[1][1], v[1][0], -v[1][3], v[1][2]  # Bds of region
                    if (lb1 <= lb1n <= ub1 or lb1 <= ub1n <= ub1 or
                            lb1n <= lb1 <= ub1n or lb1n <= ub1 <= ub1n) and \
                        (lb2 <= lb2n <= ub2 or lb2 <= ub2n <= ub2 or
                            lb2n <= lb2 <= ub2n or lb2n <= ub2 <= ub2n):  # Adjacency test
                        if not self.region_is_obstacle(v, obs_bounds):
                            neighbours[r].append(k)
                        else:
                            print "Region " + str(k) + " is an obstacle, not reachable from " + str(r) + "."
            # Then go through and add
        else:
            print "Cannot return neighbours for any state dimension other than 2!"
        return neighbours

    def manual_policy(self, x_in):
        # Manually specify a u = pi(x_in)
    ...
    def calc_ctrb_steps(A, B, eig_tol):
        ...
        for t in range(1, n+1):
            ...
            if np.linalg.matrix_rank(np.hstack((np.dot(np.linalg.matrix_power(A, i), B)
    ...
    @staticmethod
    def region_is_obstacle(r, obs_bounds_in):
        if obs_bounds_in is None:
            return False
        A, b = r
        lb1, ub1, lb2, ub2 = -b[1], b[0], -b[3], b[2]
        mid1, mid2 = 0.5 * (lb1 + ub1), 0.5 * (lb2 + ub2)
        obstacle = False
        for obs in obs_bounds_in:
            A_obs, b_obs = obs
            lb1_obs, ub1_obs, lb2_obs, ub2_obs = -b_obs[1], b_obs[0], -b_obs[3], b_obs[2]
            if lb1_obs < mid1 < ub1_obs and lb2_obs < mid2 < ub2_obs:
                obstacle = True
        return obstacle
